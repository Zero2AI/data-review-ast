...
DEFAULT_BEDROCK_MODEL = os.getenv("BEDROCK_MODEL_ID", "us.anthropic.claude-sonnet-4-20250514-v1:0")

def humanize_sql_error(error_msg: str, query: str) -> str:
    """
    Convert low-level SQL errors into user-friendly messages for non-technical users.
    """
    lower_error = (error_msg or "").lower()
    q = query or ""

    # Extract likely "hallucinated" token from bad SQL (e.g., SEQUELAE)
    tokens = [t for t in re.findall(r"[A-Za-z_][A-Za-z0-9_]*", q) if len(t) > 2]

    sql_keywords = {
        "select","from","where","with","and","or","join","group","order","limit","as",
        "distinct","count","avg","min","max","sum","case","when","then","else","end",
        "inner","left","right","on"
    }

    # Prefer non-SQL keywords (so SEQUELAE beats WITH)
    non_sql_tokens = [
        t for t in tokens
        if t.lower() not in sql_keywords
        ]

    likely_term = non_sql_tokens[0] if non_sql_tokens else (tokens[0] if tokens else None)
    ...
    if "no such column" in lower_error:
        ...
        return (
            f"The field '{missing}' is not available in the dataset. "
    ...
    if "no such table" in lower_error:
        ...
        return (
            f"The dataset does not contain a table named '{missing}'. "
            "Please verify the data source and try again."
        )

    # Syntax / token errors (common when the model invents a term)
    if "unrecognized token" in lower_error or "syntax error" in lower_error:
        if likely_term and likely_term.lower() not in sql_keywords:
            # This is the message you wanted: “sequelae not available…”
            return (
                f"The term '{likely_term}' is not available in the dataset. "
                "Please check the available fields/values or rephrase your question."
            )
        return (
            "I couldn’t run this request because part of it doesn’t match the dataset format. "
            "Please rephrase your question using fields that exist in the data."
        )
...
class SQLGenerator:    
    def __init__(self, temperature: float = 0.1, study_name: str = None):
        ...
        self.model_id = DEFAULT_BEDROCK_MODEL
...
def execute_multi_query(conn, queries: List[str]) -> Dict[str, pd.DataFrame]:
    ...
    for i, query in enumerate(queries):
        try:
            ...
            friendly = humanize_sql_error(str(e), query)
            errors[f"query_{i+1}"] = {
...
def execute_query(conn, query: str) -> pd.DataFrame:
    try:
        ...
        raise Exception(humanize_sql_error(str(e), query))
